...
def mk_test_name(name, value, index=0):
    ...
    if not is_trivial(value):
        ...
        if isinstance(value, Case): # 这里判断是否是case的实例。判断value如果是Case这个类的实例
            value = value.title # 如果是，就把value重置一下替换成title（这里的title是Case.title）
...
def ddt(cls):
    ...
    for name, func in list(cls.__dict__.items()):
        #cls.__dict__返回当前这个类的所有属性(返回类的属性值)，而且是一个字典，字典的items
        if hasattr(func, DATA_ATTR):#判断是否有DATA_ATTR
            for i, v in enumerate(getattr(func, DATA_ATTR)):#列表里面有几条数据，第二个for循环就会循环几遍
